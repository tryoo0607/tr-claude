...
개인 kit — **22개 스킬**. `tr-kit-source`에서 `build.sh claude`로 생성(수동 유지 X).
...
| `session` | 세션·작업을 이관·위임하거나 실행 중 세션의 plugin runtime을 갱신하고 원격 세션 무응답을 진단할 때 발동 — "세션 넘겨줘/롤오버/핸드오버", 컨텍스트 포화 후속 인계, "기존 세션에도 플러그인 적용", "이 작업 다른 세션/사람/에이전트에 넘겨줘", "세션이 멈췄어/반응 없어". state 기반 롤오버와 HandOff 규격으로 이어받을 경계를 남긴다. |
