---
name: kit
description: 이 kit 자체를 설치·점검·업데이트하거나 스킬·컴포넌트를 저작할 때 발동 — "kit 설치/업데이트", "스킬 만들어줘/추가해줘", "스킬 출력 형식 설계", "kit 군살 점검". 공통 저작·검증과 호스트별 배포 채널을 연결한다.
---
...
Claude kit은 plugin cache와 셸 설치본을 서로 다른 채널로 다룬다. version·marketplace·cache·재시작과 사용자 설정 보존 절차는 `references/delivery.md`를 읽는다. 실제 배포·설치·재시작은 diff와 영향을 보고하고 승인 후 수행한다.
